...
func TestMediumPatternMatching(t *testing.T) {
	testhelpers.WithTestContext(t, 10*time.Second, func(t *testing.T, tmpDir string) {
		// Reproduce the exact medium pattern structure
		configContent := `
settings:
  project_id: "medium-debug"
  verbose: true
...
`
		...
		// Create directory structure matching medium
