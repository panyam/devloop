...
// Use this to monitor execution progress and check for failures.
type RuleStatus struct {
	...
	// Set when rule begins executing, cleared when rule finishes
	...
	// When the most recent execution finished (success or failure)
	// Use this to check how recently a rule completed execution
	LastFinished *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=last_finished,json=lastFinished,proto3" json:"last_finished,omitempty"`
	...
	// Only set when last_build_status = "FAILED"
	// Contains the actual error message that caused the failure
...
func (x *RuleStatus) GetLastStarted() *timestamppb.Timestamp {
	if x != nil {
...
func (x *RuleStatus) GetLastFinished() *timestamppb.Timestamp {
	if x != nil {
		return x.LastFinished
	}
	return nil
}
